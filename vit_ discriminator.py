...
class PatchEncoder(torch.nn.Module):
    def __init__(self, num_patches=64, projection_dim=16):
        super(PatchEncoder, self).__init__()
        self.num_patches = num_patches
        # self.projection = torch.nn.Linear(
        #     64*64*4, out_features=projection_dim)  # 256*256:262144
        self.projection = torch.nn.Linear(
            24567, out_features=projection_dim)  # 256*256:262144
        self.projection_resize = torch.nn.Linear(
            32*32*4, out_features=projection_dim)
    ...
    def forward(self, input):
        ...
        if input.shape[-1] == 4096:
            encoded = self.projection_resize(
...
class Block(torch.nn.Module):
    def __init__(self, project_dim, depth, num_heads, mlp_ratio):
        ...
        self.depth = depth
        self.normlayer1 = torch.nn.LayerNorm(
            normalized_shape=project_dim, eps=1e-6)
        self.normlayer2 = torch.nn.LayerNorm(
            normalized_shape=project_dim, eps=1e-6)
        ...
        for i in range(depth):
            setattr(self, "layer"+str(i+1),
                    torch.nn.MultiheadAttention(project_dim, num_heads, dropout=0.1))

    def forward(self, encoded_patches):
        feat = []
        for i in range(self.depth):
            x1 = self.normlayer1(encoded_patches)
            attention_output, attn_output_weights = getattr(
                self, "layer"+str(i+1))(x1, x1, x1)
...
class vit_discriminator(torch.nn.Module):
    def __init__(self, patch_size, project_dim=64, num_heads=4, mlp_ratio=2, depth=4):
        ...
        self.GELU = torch.nn.GELU()
        self.block = Block(project_dim=project_dim, depth=depth,
                           num_heads=num_heads, mlp_ratio=mlp_ratio).cuda()
        ...
        self.MultiHeadAttention = torch.nn.MultiheadAttention(
            project_dim, num_heads, dropout=0.1)
        ...
        self.LayerNormalization_0 = torch.nn.LayerNorm(
            normalized_shape=project_dim, eps=1e-6)
        self.LayerNormalization_1 = torch.nn.LayerNorm(
            normalized_shape=project_dim, eps=1e-6)
        self.LayerNormalization_2 = torch.nn.LayerNorm(
            normalized_shape=project_dim, eps=1e-6)
    ...
    def forward(self, fundus, angio):
        ...
        feat = []
        patches = rearrange(
            X, 'b c (h h1) (w w2) -> b (h w) (h1 w2 c)', h1=patch_size, w2=patch_size)
        ...
        features = self.AdaptiveAvgPool2d(
            representation).squeeze(-1).squeeze(-1)
...
if __name__ == '__main__':
    ...
    # content_images = torch.randn(4,3,256,256).cuda()
    # style_images = torch.randn(4,3,256,256).cuda()
    
    content_images = torch.randn(4, 3,256,256).cuda()
    style_images = torch.randn(4, 3,256,256).cuda()
    
    discriminator = vit_discriminator(64)
    d_feat1_real = discriminator(content_images, style_images)
    print(d_feat1_real)
    print(d_feat1_real.shape)
