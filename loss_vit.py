...
from contra_loss import PatchLoss, ConstLoss
...
'''
ying************************************************************************************************
the class Loss_vit need to be instantiate(实例化) and the be used
return the vit_loss--ref DiffuseIT
'''


class Loss_vit(torch.nn.Module):

    def __init__(self, lambda_ssim=1.0, lambda_dir_cls=1.0, lambda_contra_ssim=1.0, lambda_trg=0.0):
        super().__init__()
        ...
        '''
        ying********************************************************************************************
        need to change the parameters of the VitExtractor--model name is related to sum params of vit/DINO like layers and num_heads
        and the parameters should be defined to decided which loss will be used--lambda_ssim=1.0,lambda_dir_cls=1.0,lambda_contra_ssim=1.0,lambda_trg=0.0
        '''
        # self.extractor = VitExtractor(
        #     model_name=cfg['dino_model_name'], device=device)
        self.extractor = VitExtractor(device=device)

        imagenet_norm = transforms.Normalize(
            (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        # global_resize_transform = Resize(
        #     cfg['dino_global_patch_size'], max_size=480)

        '''
        ying****************************************************************************************
        tranform the images using normalization and resize
        '''
        # self.global_transform = transforms.Compose([global_resize_transform,
        #                                             imagenet_norm
        #                                             ])
        self.global_transform = transforms.Compose([imagenet_norm])
    ...
    def forward(self, outputs, source, out_prev=None, use_dir=True, target=None, frac_cont=1.0):
        ...
        if self.lambdas['lambda_global_ssim'] > 0:
            losses['loss_global_ssim'] = self.calculate_global_ssim_loss(
                outputs, source)
            loss_G += losses['loss_global_ssim'] * \
                self.lambdas['lambda_global_ssim']
            losses_val['loss_global_ssim'] = losses['loss_global_ssim'].item()
        if self.lambdas['lambda_contra_ssim'] > 0:
            losses['loss_contra_ssim'] = self.calculate_contra_ssim_loss(
                outputs, source)
            loss_G += losses['loss_contra_ssim'] * \
                self.lambdas['lambda_contra_ssim']*frac_cont
            losses_val['loss_contra_ssim'] = losses['loss_contra_ssim'].item()
        if use_dir:
            if self.lambdas['lambda_dir_cls'] > 0:
                losses['loss_dir_cls'] = self.calculate_dir_cls_loss(
                    outputs, out_prev)
                loss_G += losses['loss_dir_cls'] * \
                    self.lambdas['lambda_dir_cls']
                losses_val['loss_dir_cls'] = losses['loss_dir_cls'].item()
        if target is not None:
            if self.lambdas['lambda_trg'] > 0:
                losses['loss_trg'] = self.calculate_target_loss(
                    outputs, target)
    ...
    def calculate_dir_cls_loss(self, outputs, out_prev):
        ...
        for a, b in zip(outputs, out_prev):  # avoid memory limitations
            ...
            with torch.no_grad():
                prev_cls_token = self.extractor.get_feature_from_input(
                    b)[-1][0, 0, :]
    ...
    def calculate_target_loss(self, outputs, target):
        ...
        for a, b in zip(outputs, target):  # avoid memory limitations
            ...
            cls_token = self.extractor.get_feature_from_input(a)[-1][0, 0, :]
            with torch.no_grad():
                targ_cls_token = self.extractor.get_feature_from_input(
                    b)[-1][0, 0, :]
    ...
    def calculate_contra_ssim_loss(self, outputs, inputs):
        ...
        for a, b in zip(inputs, outputs):  # avoid memory limitations
            ...
            b = self.global_transform(b)
            with torch.no_grad():
                target_keys = self.extractor.get_keys_from_input(
                    a.unsqueeze(0), 11)
                h, t, d = target_keys.shape
                concatenated_target = target_keys.transpose(
                    0, 1).reshape(t, h * d)
            keys = self.extractor.get_keys_from_input(b.unsqueeze(0), 11)
            h, t, d = keys.shape
            concatenated_keys = keys.transpose(0, 1).reshape(t, h * d)

            loss += self.patch_loss(concatenated_keys,
                                    concatenated_target).mean()
...
if __name__ == "__main__":
    loss_G, loss_val = Loss_vit()
    print(loss_G, loss_val)
